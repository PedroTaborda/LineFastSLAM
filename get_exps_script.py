...
action_model_noise_var_r_exp = [
    ...
    for r_var in np.linspace(0.05, 0.5, 10, endpoint=True)
]
action_model_noise_var_theta_exp = [
    [r_odom_default, theta_var]
    for theta_var in np.linspace(0.1, 1.0, 10, endpoint=True)
]

r_line = " ".join([f"{r_line_val:.7f}" for r_line_val in np.linspace(0.05, 0.5, 10, endpoint=True)])

phi_line = " ".join([f"{phi_line_val:.7f}" for phi_line_val in np.linspace(2*np.pi/180, 20*np.pi/180, 10, endpoint=True)])
